...
final class Util implements Opcodes {
    ...
    public static <T> T createAsmProxy(final Class<T> proxy) {
        ...
        try {
            MethodHandles.Lookup lookup = MethodHandles.privateLookupIn(targetClass, NyanaReflection.getLookup());
            ...
            for (MethodHandle finalFieldHandle : builder.finalFields()) {
                ...
                NyanaReflection.setAccessible(handleField).set(null, finalFieldHandle);
            }
    ...
    private static Class<?> getProxiedClass(Class<?> clazz, ReflectionProxy proxy, ClassLoader classLoader) {
        ...
        if (proxy.ignoreRelocation()) {
            ...
            if (name.length == 1) {
                return NyanaClass.find(false, classLoader, name[0].replace("{}", "."));
            } else {
                return NyanaClass.find(false, classLoader, Arrays.stream(name).map(it -> it.replace("{}", ".")).toArray(String[]::new));
            }
        } else {
            return NyanaClass.find(false, classLoader, proxy.name());
        }
    }
    ...
    public static Class<?> getParameterClass(Parameter parameter, ClassLoader classLoader) {
        ...
        if (type.ignoreRelocation()) {
            ...
            return NyanaClass.find(false, classLoader, type.name());
        }
    }
    ...
    private static void analyseAndApply(Class<?> proxyClass, Class<?> targetClass, ProxyBuilder builder) {
        NyanaClass<?> spaClass = new NyanaClass<>(targetClass);
        for (Method method : proxyClass.getDeclaredMethods()) {

            // 字段获取
            FieldGetter fieldGetter = method.getAnnotation(FieldGetter.class);
            if (fieldGetter != null && NyanaReflection.getActivePredicate().test(fieldGetter.activeIf())) {
                // 字段只需 名称 即可确定
            ...
            if (fieldSetter != null && NyanaReflection.getActivePredicate().test(fieldSetter.activeIf())) {
            ...
            if (methodInvoker != null && NyanaReflection.getActivePredicate().test(methodInvoker.activeIf())) {
            ...
            if (constructorInvoker != null && NyanaReflection.getActivePredicate().test(constructorInvoker.activeIf())) {
                // 构造器只需要 参数 即可确定
                Class<?>[] parameterTypes = Arrays.stream(method.getParameters())
                        .map(p -> Util.getParameterClass(p, proxyClass.getClassLoader()))
                        .toArray(Class<?>[]::new);
