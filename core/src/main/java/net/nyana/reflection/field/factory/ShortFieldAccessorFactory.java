...
import net.nyana.reflection.field.primitive.SShortField;
...
// short 字段的 ASM 访问器工厂. 为目标字段生成隐藏类, 直接以 short 读写, 免去装箱/拆箱开销.
@SuppressWarnings("DuplicatedCode")
final class ShortFieldAccessorFactory implements Opcodes {
    private ShortFieldAccessorFactory() {}
    private static final String ABSTRACT_CLASS_INTERNAL_NAME = Type.getInternalName(SShortField.class); // 生成类继承的抽象基类内部名

    // 校验字段类型后生成字节码, 定义为宿主类的嵌套隐藏类并实例化返回.
    static SShortField create(Field field) throws Exception {
        // 仅接受 short 字段, 类型不符直接拒绝
